...
void cc_senddcw_cli(struct cc_client_data *cli)
{
	...
	if ( (ecm->dcwstatus==STAT_DCW_SUCCESS)&&(ecm->hash==cli->ecm.hash) ) {
		...
		//debugf(" |> decode failed to CCcam client '%s' ecm status %s ch %04x:%06x:%04x (%dms)\n", cli->user, ecm->statusmsg, ecm->caid,ecm->provid,ecm->sid, GetTickCount()-cli->ecm.recvtime);
	}
	cli->ecm.busy=0;
