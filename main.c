...
#else
char config_file[256] = "/var/multics/multics.e0.cfg";
#endif
...
{
	//messi89 : ecmlen 431 support
	//if ( (ecmlen<20)||(ecmlen>300) ) return 0;
	if ( (ecmlen<20)||(ecmlen>1024) ) return 0;
	return 1;
}
...
int mainprocess()
{
	...
	debugf( " Multi CardServer r%d - by evileyes (email:evileyes@live.fr) - modified by Messi89 (skype: messi89sat)\n", REVISION );
...
int main(int argc, char *argv[])
{
	...
	// Set Config name
	sprintf( config_file, "/var/multics/%s.e0.cfg", path);
...
OPTIONS\n\
\t-b               run in background\n\
\t-C <configfile>  use <configfile> instead of default config file (/var/multics/multics.e0.cfg)\n\
