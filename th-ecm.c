...
int srvtab_arrange(struct cardserver_data *cs, int ecmid, int bestone )
{
	...
	//debugf(" 86 srvtab_arrange(%04x:%06x:%04x) %d\n", ecm->caid, ecm->provid, ecm->sid, nbsrv);
...
//		if ( srvtab[j].val > -100 )
	...
	//debugf(" 126 srvtab_arrange(%04x:%06x:%04x) %d\n", ecm->caid, ecm->provid, ecm->sid, nbsrv);
...
*/
	...
	//debugf(" 200 srvtab_arrange(%04x:%06x:%04x) %d\n", ecm->caid, ecm->provid, ecm->sid, nbsrv);
	...
	//debugf(" 215 srvtab_arrange(%04x:%06x:%04x) %d\n", ecm->caid, ecm->provid, ecm->sid, nbsrv);
	...
	//debugf(" 347 srvtab_arrange(%04x:%06x:%04x) %d\n", ecm->caid, ecm->provid, ecm->sid, nbsrv);
...
//XXX ...
	...
	while ( (ecmid=prevecmid(ecmid)) != lastecmid ) {
		...
		if ( (ecm->dcwstatus==STAT_DCW_WAIT) ) {
			...
			// Check for ECM sending messi89 bug
