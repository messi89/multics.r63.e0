...
char http_body[] = "<BODY onload=\"start();\">\n";
char http_body_[] = "</BODY>\n";

char http_title[] = "<title>MultiCS.E0 - %s</title>\n";
...
char http_menu[] = "<div class=\"menu\"><span class='button'><a href='/'>Home</a></span>\
...
REVISION_STR
"- Modified by MESSI89</b></span></div><br><br>\n";

char http_refresh[] = "<div align=right>Autorefresh <select onchange=\"setautorefresh(this.value);\"><option value=0>OFF</option><option value=5000>5s</option><option value=10000>10s</option><option value=15000>15s</option><option value=20000>20s</option><option value=30000 selected>30s</option><option value=60000>60s</option></select></div>\n";

char http_script[] = "\n<script type=\"text/JavaScript\">function setautorefresh(e){clearTimeout(tautorefresh),autorefresh=e,e>0&&(tautorefresh=setTimeout(\"updateDiv()\",autorefresh))}function updateDiv(){var e;try{e=new XMLHttpRequest}catch(t){try{e=new ActiveXObject(\"Msxml2.XMLHTTP\")}catch(r){try{e=new ActiveXObject(\"Microsoft.XMLHTTP\")}catch(o){e=!1}}}return e?(e.onreadystatechange=function(){4==e.readyState&&(200==e.status&&(requestError=0,document.getElementById(\"mainDiv\").innerHTML=e.responseText),tautorefresh=setTimeout(\"updateDiv()\",autorefresh))},e.open(\"GET\",\"/?action=div\",!0),void e.send(null)):(alert(\"Your browser does not support Ajax.\"),!1)}function start(){setautorefresh(autorefresh)}var autorefresh=3e4,tautorefresh;</script>\n";
...
void http_send_index(int sock, http_request *req)
{
	...
	tcp_write(&tcpbuf, sock, http_script, strlen(http_script) );
	...
	tcp_write(&tcpbuf, sock, http_refresh, strlen(http_refresh) );
	

	unsigned int d= GetTickCount()/1000;
	sprintf( http_buf,"<div id=\"mainDiv\"><br>Uptime: %02dd %02d:%02d:%02d", d/(3600*24), (d/3600)%24, (d/60)%60, d%60); tcp_write(&tcpbuf, sock, http_buf, strlen(http_buf) );
	...
	sprintf( http_buf, "</pre></div>"); tcp_write(&tcpbuf, sock, http_buf, strlen(http_buf) );
...
void http_send_editor(int sock, http_request *req)
{
	...
	if (req->type==HTTP_POST) {
		...
		sprintf( http_buf, "<form enctype=\"multipart/form-data\"action=\"/editor\" method=\"post\"><input type=\"submit\" value=\"Save '%s'\"><br><center><textarea cols=\"40\" rows=\"10\" spellcheck=\"false\" name=\"textedit\">",config_file); tcp_write(&tcpbuf, sock, http_buf, strlen(http_buf) );
		...
		sprintf( http_buf, "</textarea><br></center></form>");
		tcp_write(&tcpbuf, sock, http_buf, strlen(http_buf) );
		tcp_flush(&tcpbuf, sock);
	}
}
